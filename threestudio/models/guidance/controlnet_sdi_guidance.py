...
from diffusers import DDIMScheduler, DDIMInverseScheduler, StableDiffusionPipeline, StableDiffusionControlNetPipeline
...
@threestudio.register("controlnet-sdi-guidance")
class ControlNetSDIGuidance(BaseObject):
    @dataclass
    class Config(BaseObject.Config):
        pretrained_model_name_or_path: str = "stabilityai/stable-diffusion-2-1-base"
        controlnet_model_name_or_path: str = "thibaud/controlnet-sd21-scribble-diffusers"
    ...
    def configure(self) -> None:
        threestudio.info(f"Loading Stable Diffusion and ControlNet ...")
    ...
    def predict_noise(
        ...
        condition = None, # ControlNet condition
    ):
        ...
        if prompt_utils.use_perp_neg:
            ...
            if condition is None:
                ...
                condition = torch.cat([condition] * 4, dim=0)
                (
                    down_block_res_samples, # batch_size = B
            ...
            noise_pred_text = noise_pred[:batch_size] # text & condition
            noise_pred_uncond = noise_pred[batch_size : batch_size * 2] # no text & no condition
            noise_pred_neg = noise_pred[batch_size * 2 :] # negative text & no condition
    ...
    def compute_grad_sdi(
        ...
        condition : Float[Tensor, "B H W C"] = None,
    ):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if condition is not None:
            condition = self.pipe.control_image_processor.preprocess(condition, height=512, width=512).to(dtype=torch.float32)
            condition = condition.to(device=self.device, dtype=self.weights_dtype)
        ...
        for b, i in enumerate(idxs):
            ...
            for t in tqdm(self.scheduler.timesteps[i + 1 :], leave=False):
                ...
                noise_pred, _, _ = self.predict_noise(
                    latents, t, prompt_utils, elevation, azimuth, camera_distances, guidance_scale=1.0, condition=condition
                )
                # get prev latent
